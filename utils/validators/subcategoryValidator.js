...
exports.createSubcategoryValidator = [
    check('name').exists().withMessage('Name field is required').notEmpty().withMessage('Name field must be not empty').isLength({ min: 2 }).withMessage("Subcategory name must be longer than 3 chars")
        .isLength({ max: 32 }).withMessage('Subcategory name must be shorter than 32 chars').custom((val, { req }) => {
...
exports.updateSubcategoryValidator = [
    ...
    check('name').exists().withMessage('subcategory id is required').isMongoId().withMessage('subcategory id is invalid').custom((val, { req }) => {
        req.body.slug = slugify(val);
        return true;
    }),
